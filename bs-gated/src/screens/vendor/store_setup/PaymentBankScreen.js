...
export default function PaymentBankScreen({ navigation }) {
  const theme = useTheme();
  const [upi,     setUpi]     = useState('freshmart@upi');
  const [account, setAccount] = useState('XXXX XXXX 4567');
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <AppHeader title="Payment & Bank Details" onBack={() => navigation.goBack()} />
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>

        <Card style={{ marginBottom: 12 }}>
          <Text style={s.sectionLabel}>UPI Details</Text>
          <InputField label="UPI ID" value={upi} onChangeText={setUpi} placeholder="yourstore@upi" />
        </Card>
        ...
        <Card style={{ marginBottom: 12 }}>
          ...
          <InputField label="IFSC Code"      value={ifsc}     onChangeText={setIfsc}    placeholder="IFSC code"                               />
