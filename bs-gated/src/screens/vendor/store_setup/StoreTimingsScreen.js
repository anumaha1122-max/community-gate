...
export default function StoreTimingsScreen({ navigation }) {
  const theme = useTheme();
  const DAYS = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
  const [open, setOpen] = useState({ Monday: true, Tuesday: true, Wednesday: true, Thursday: true, Friday: true, Saturday: true, Sunday: false });

  return (
    <SafeAreaView style={s.safe}>
      <StatusBar barStyle={theme.mode === 'light' ? 'dark-content' : 'light-content'} backgroundColor={theme.header} />
      <AppHeader title="Store Timings" onBack={() => navigation.goBack()} />
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          ...
          {DAYS.map((day, i) => (
            <View key={day}>
              <View style={s.toggleRow}>
                <View style={{ flex: 1 }}>
                  <Text style={s.toggleLabel}>{day}</Text>
                  <Text style={s.toggleSub}>{open[day] ? '8:00 AM – 10:00 PM' : 'Closed'}</Text>
                </View>
                <Switch
                  value={open[day]}
                  onValueChange={(v) => setOpen(prev => ({ ...prev, [day]: v }))}
        ...
        <PrimaryButton title="Save Timings" onPress={() => navigation.goBack()} color={Colors.teal} />
      </ScrollView>
    </SafeAreaView>
  );
}
