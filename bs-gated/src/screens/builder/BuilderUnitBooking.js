    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerTop}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Flat Bookings</Text>
          <Text style={styles.headerSub}>Customer booking approvals</Text>
        </View>
      </View>
    </View>
  );
      ...
      <View style={styles.bookingTop}>
        ...
        <View style={styles.bookingMain}>
          ...
          <Text style={styles.bookingSub}>
            {booking?.projectName || "Project"} • {booking?.unitNo || booking?.unitNumber || "Unit"} •{" "}
...
export default function BuilderUnitBooking({ navigation }) {
  ...
  const stats = useMemo(
    () => ({
      total: flatBookingRequests.length,
      pending: flatBookingRequests.filter((b) => b?.status === "Pending Approval").length,
      approved: flatBookingRequests.filter((b) => b?.status === "Approved").length,
      rejected: flatBookingRequests.filter((b) => b?.status === "Rejected").length,
    }),
    [flatBookingRequests]
  );
  ...
  }, [flatBookingRequests, filter, search]);
  ...
  const handleVerifyDocs = () => {
    ...
    verifyFlatBookingDocuments(selectedBooking.id);
    setSelectedBooking((prev) => (prev ? { ...prev, documentStatus: "Verified" } : prev));
  };

  const handlePaymentReceived = () => {
    if (!selectedBooking?.id) return;
    markFlatBookingPaymentReceived(selectedBooking.id);
    setSelectedBooking((prev) => (prev ? { ...prev, paymentStatus: "Received" } : prev));
  };
          ...
          <View style={styles.bookingList}>
            {filteredBookings.length ? (
              ...
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptySub}>Customer booking requests will appear here.</Text>
              </View>
            )}
          </View>
        </ScrollView>
